# UCloud vLLM Pre-flight Checklist (MIG / Blackwell) — vLLM track

> **vLLM-track layout:** experiment artifacts under `/work/LCPP_OffloadTesting/vLLM/`; CUDA toolchain shared w/ `/work/LCPP_OffloadTesting/hpc/toolchain/` by path (do not duplicate). Update paths here if toolchain moves.
...
```bash
export CUDA_HOME=/work/LCPP_OffloadTesting/hpc/toolchain/cuda-13   # shared w/ hpc track
...
| path | content |
|---|---|
| `vLLM/venvs/` | per-experiment venvs (nightly-Gemma4, stable-0.27, …) |
| `vLLM/models/` | HF caches for vLLM-track checkpoints |
| `vLLM/data/` | V7 prompt suites (copied from hpc) |
| `vLLM/experiments/` | per-date artifacts + logs + README postmortems |
| `vLLM/flashinfer_cache/` | JIT cache (persist from `~/.cache/flashinfer` after first serve) |
| `hpc/toolchain/{cuda-13,pkg}` | shared sm100 toolchain + debs |
| `hpc/models/` | earlier sweeps (LFM2.5, Qwen3.8-NVFP4) |
| `hpc/venvs/qwen38_vllm` | vLLM ≥0.27 stable (non-Gemma4 fallback) |
